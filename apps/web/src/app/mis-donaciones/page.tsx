 ...
 * across all emergencies, so they follow their donations from their account
 * without keeping a code. Each row links to the public tracking page. The
 * operator's code/email search is a separate, permission-gated surface.
 */
export default async function MisDonacionesPage() {
  const token = await getToken();
  if (token === null) {
    redirect('/login?next=/mis-donaciones');
  }
  ...
  return (
    <main className="flex-1 bg-surface">
      <div className="mx-auto w-full max-w-md">
        <PageHeaderBand
          backHref="/panel"
          backLabel={tm.back_to_panel}
          title={tm.page_title}
          subtitle={tm.page_subtitle}
        />
        <div className="flex flex-col gap-4 px-4 pb-12 pt-6">
          {donations.length === 0 ? (
