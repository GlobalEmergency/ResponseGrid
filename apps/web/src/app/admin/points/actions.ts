...
export async function fetchAdminResources(): Promise<ResourceAdminListItem[]> {
  const token = await requireSession('/admin/points');
...
export async function fetchAdminResourceDetail(
  ...
  const token = await requireSession(`/admin/points/${id}`);
...
export async function recordInventoryEntry(
  ...
  const token = await requireSession(`/admin/points/${resourceId}`);
  ...
  revalidatePath(`/admin/points/${resourceId}`);
